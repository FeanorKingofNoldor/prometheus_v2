...
erDiagram
    %% Core entities
    markets ||--o{ instruments : contains
    issuers ||--o{ instruments : issues
    issuers ||--o{ profiles : has
    
    %% Market data
    instruments ||--o{ prices_daily : has
    instruments ||--o{ returns_daily : has
    instruments ||--o{ volatility_daily : has
    instruments ||--o{ instrument_factors_daily : exposed_to
    factors_daily ||--o{ instrument_factors_daily : defines
    
    %% Text & events
    news_articles ||--o{ news_links : mentions
    news_links }o--|| issuers : about
    issuers ||--o{ filings : files
    issuers ||--o{ earnings_calls : hosts
    
    %% Decision flow
    engine_configs ||--o{ engine_decisions : used_by
    engine_decisions ||--o{ executed_actions : generates
    engine_decisions ||--o{ decision_outcomes : evaluated
    engine_decisions ||--o{ orders : creates
    
    %% Execution
    portfolios ||--o{ orders : places
    portfolios ||--o{ positions_snapshots : holds
    instruments ||--o{ orders : traded
    orders ||--o{ fills : filled_by
    
    %% Engine outputs
    strategies ||--o{ instrument_scores : scores
    strategies ||--o{ universes : defines
    strategies ||--o{ target_portfolios : targets
    portfolios ||--o{ target_portfolios : has
    portfolios ||--o{ portfolio_risk_reports : reports
    
    %% Scenarios
    scenario_sets ||--o{ scenario_paths : contains
    instruments ||--o{ scenario_paths : simulated
    ...
    instruments ||--o{ returns_daily : "has"
    instruments ||--o{ volatility_daily : "has"
    instruments ||--o{ instrument_factors_daily : "has"
    instruments ||--o{ orders : "traded_via"
    instruments ||--o{ fills : "filled"
    instruments ||--o{ positions_snapshots : "held_in"
    instruments ||--o{ instrument_scores : "scored"
    instruments ||--o{ scenario_paths : "simulated"
    
    instruments }o--|| issuers : "issued_by"
    instruments }o--|| markets : "trades_in"
    
    issuers ||--o{ profiles : "has_profile"
    issuers ||--o{ news_links : "mentioned_in"
    issuers ||--o{ filings : "files"
    issuers ||--o{ earnings_calls : "hosts"
    
    news_articles ||--o{ news_links : "links_to"
    
    factors_daily ||--o{ instrument_factors_daily : "exposes"
    
    engine_decisions ||--o{ executed_actions : "leads_to"
    engine_decisions ||--o{ decision_outcomes : "evaluated_as"
    engine_decisions ||--o{ orders : "generates"
    
    portfolios ||--o{ executed_actions : "executed_in"
    portfolios ||--o{ orders : "orders_for"
    portfolios ||--o{ positions_snapshots : "positions_in"
    portfolios ||--o{ target_portfolios : "targets"
    portfolios ||--o{ portfolio_risk_reports : "risk_of"
    
    strategies ||--o{ instrument_scores : "scores_for"
    strategies ||--o{ universes : "defines"
    strategies ||--o{ target_portfolios : "targets"
